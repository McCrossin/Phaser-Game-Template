...
class PerformanceMonitor {
    ...
    private loadThresholds(): PerformanceThresholds {
        // Check if we're running in template validation mode
        const isTemplateValidation = process.env['TEMPLATE_VALIDATION'] === 'true';

        return {
            minFPS: isTemplateValidation ? 5 : this.isCI ? 2 : 30,
    ...
    private async withRetry<T>(
        ...
        for (let attempt = 1; attempt <= this.config.retry.maxAttempts; attempt++) {
            try {
                this.logInfo(
                    `${operationName} - Attempt ${attempt}/${this.config.retry.maxAttempts}`
                );
                ...
                if (attempt < this.config.retry.maxAttempts) {
                    ...
                    this.logError(
                        `${operationName} failed after ${this.config.retry.maxAttempts} attempts: ${lastError.message}`
                    );
                }
            }
        }
    ...
    async checkBundleSize(): Promise<boolean> {
        return (
            (await this.withRetry(async () => {
                ...
                console.log(
                    `Bundle size: ${bundleSizeMB.toFixed(2)}MB (threshold: <${thresholdMB.toFixed(2)}MB)`
                );
                ...
                if (bundleSize > this.thresholds.maxBundleSize) {
                    throw new Error(
                        `Bundle size exceeds limit: ${bundleSizeMB.toFixed(2)}MB > ${thresholdMB.toFixed(2)}MB`
                    );
                }

                if (bundleSize > this.thresholds.maxBundleSize * 0.8) {
                    this.logWarning(
                        `Bundle size approaching limit: ${bundleSizeMB.toFixed(2)}MB (${((bundleSize / this.thresholds.maxBundleSize) * 100).toFixed(1)}% of limit)`
                    );
                }
        ...
        return (
            (await this.withRetry(async () => {
                ...
                if (resultAge > 24 * 60 * 60 * 1000) {
                    this.logWarning(
                        `Performance results are ${Math.floor(resultAge / (60 * 60 * 1000))} hours old`
                    );
                }
                ...
                if (results.fps && typeof results.fps.average === 'number') {
                    ...
                    console.log(
                        `Average FPS: ${avgFPS.toFixed(2)} (threshold: >${this.thresholds.avgFPS})`
                    );
                    console.log(
                        `Minimum FPS: ${minFPS.toFixed(2)} (threshold: >${this.thresholds.minFPS})`
                    );

                    if (avgFPS < this.thresholds.avgFPS) {
                        throw new Error(
                            `Average FPS below threshold: ${avgFPS.toFixed(2)} < ${this.thresholds.avgFPS}`
                        );
                    }

                    if (minFPS < this.thresholds.minFPS) {
                        throw new Error(
                            `Minimum FPS below threshold: ${minFPS.toFixed(2)} < ${this.thresholds.minFPS}`
                        );
                    }
                    ...
                    if (results.fps.baseline && typeof results.fps.baseline === 'number') {
                        ...
                        console.log(
                            `FPS degradation: ${(degradation * 100).toFixed(2)}% (threshold: <${(this.thresholds.degradationTolerance * 100).toFixed(1)}%)`
                        );

                        if (degradation > this.thresholds.degradationTolerance) {
                            throw new Error(
                                `FPS degradation exceeds tolerance: ${(degradation * 100).toFixed(2)}% > ${(this.thresholds.degradationTolerance * 100).toFixed(1)}%`
                            );
                        }
                    }
                }
                ...
                if (results.microfreezes && typeof results.microfreezes.count === 'number') {
                    ...
                    console.log(
                        `Microfreezes detected: ${freezeCount} (threshold: <${this.thresholds.maxMicrofreezes})`
                    );

                    if (freezeCount > this.thresholds.maxMicrofreezes) {
                        throw new Error(
                            `Too many microfreezes detected: ${freezeCount} > ${this.thresholds.maxMicrofreezes}`
                        );
                    }

                    if (freezeCount > 0 && freezeCount <= this.thresholds.maxMicrofreezes) {
                        this.logWarning(
                            `${freezeCount} microfreezes detected (within acceptable range)`
                        );
                    }
                }
...
// Script entry point for ES modules
...
export type { PerformanceConfig, MonitoringResult };
